...
#L1 Logistic regress. Note that elasticNetParam is set to 1
...
L1_features = "\n".join([spam_names[i] for i in np.argsort(np.abs(w_L1))[:5]])

print(f'5 most relevant features L1 features: \n{L1_features}')

#L2 Logistic regress. Note that elasticNetParam is set to 0
...
#We then use those index span_names with those indices to get back the feature names
#and join them together with '\n's into a single string for printing
L2_features = "\n".join([spam_names[i] for i in np.argsort(np.abs(w_L2))[:5]])
...
#We then use those index span_names with those indices to get back the feature names
#and join them together with '\n's into a single string for printing
EN_features = "\n".join([spam_names[i] for i in np.argsort(np.abs(w_EN))[:5]])
